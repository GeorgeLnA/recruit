          ...
          >
            <div className="pointer-events-auto flex flex-col items-center">
              <div className="relative">
                <span className="absolute -inset-1 rounded-full bg-[#ff3a34] opacity-30 animate-ping" />
                <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="relative block w-2.5 h-2.5 rounded-full bg-[#ff3a34] shadow ring-2 ring-white/70"
                aria-label={`Select ${pin.name}`}
              />
              </div>
              <div className="mt-1 px-1 rounded text-[10px] font-semibold text-white/90 bg-black/50 whitespace-nowrap" style={{ fontFamily: "Milker" }}>
                {pin.name}
              </div>
              {selectedId === pin.id && (
                <div className="mt-2 w-64 bg-white rounded-xl border border-gray-200 shadow-xl overflow-hidden">
                  ...
                  <div className="p-3">
                    <div className="text-sm font-bold mb-1" style={{ fontFamily: "Milker" }}>
                      {pin.name}
                    </div>
